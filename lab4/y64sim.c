...
cc_t compute_cc(alu_t op, long_t argA, long_t argB, long_t val) {
    bool_t zero = (val == 0);
    /* TODO: Why convert val to int? */
    bool_t sign = ((int)val < 0);
    bool_t ovf = FALSE;
    switch (op) {
        case A_ADD:
            ovf = (argB > 0 && argA > 0 && val < 0) ||
                  (argA < 0 && argB < 0 && val >= 0);
            break;
        case A_SUB:
            ovf = (argA < 0 && argB > 0 && val < 0) ||
                  (argA > 0 && argB < 0 && val > 0);
            break;
        default:
            break;
...
bool_t cond_doit(cc_t cc, cond_t cond) {
    ...
    switch (cond) {
        ...
        case C_E:
            doit = zf;
            break;
        /* if negation is operated, it's necessary to get the last bit */
        case C_NE:
            doit = (~zf) & 0x1;
            break;
        case C_GE:
...
stat_t nexti(y64sim_t* sim) {
    ...
    /* or use switch to simplify */
    if (icode == I_RRMOVQ || icode == I_IRMOVQ || icode == I_RMMOVQ ||
        icode == I_MRMOVQ || icode == I_ALU || icode == I_POPQ ||
        icode == I_PUSHQ) {
        if (!get_byte_val(sim->m, next_pc, &regCode)) {
            err_print("PC = 0x%lx, Invalid instruction address", sim->pc);
            return STAT_ADR;
        }
        regA = GET_REGA(regCode);
        regB = GET_REGB(regCode);
        /* check register id */
        next_pc++;
    }
    ...
    switch (icode) {
        ...
        case I_PUSHQ: /* A:0 regA:F */
            ...
            valB = get_reg_val(sim->r, REG_RSP);
            valE = valB + (-8);
            /* TODO Question: why need to set register first?? */
            set_reg_val(sim->r, REG_RSP, valE);
            if (!set_long_val(sim->m, valE, valA)) {
                err_print("PC = 0x%lx, Invalid stack address 0x%lx", sim->pc,
                          valE);
